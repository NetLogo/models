...
to setup
  ...
  set initial-gas-temperature 25
  set plunger-move 0
...
to move-plunger
  ask forces [
    set plunger-move plunger-move + my-f ;.001
  ]
  ...
  if plunger-move != 0 [
    ...
    ifelse (plunger-move > 0) [
      if ( py < syringe-top-y - 1 ) [
        ask particles with [ xcor > syringe-left-x and xcor < syringe-right-x and (abs(ycor - oy) + abs(py - ycor) = abs(py - oy)) ] [
          set ycor py + .5
        ]
...
to mouse-action
  ...
  if mouse-inside? [
    ifelse (mouse-xcor < box-max-pxcor - 1) [
      ask erasers [
      ...
      if mouse-down? [
        ask patches with [pxcor = snap-xcor and pycor = snap-ycor] [
          ...
          if mouse-interaction = "choose heat/cool region" [
...
to setup-syringe
  if not any? plungers [
    ...
    ask patches with [ ((pxcor = syringe-left-x or pxcor = syringe-right-x or pxcor = syringe-left-x + 1 or pxcor = syringe-right-x - 1) and
      (pycor = syringe-bottom-y - 1 or pycor = syringe-top-y + 1)) ]
      [ sprout-walls 1 [ set color gray - 2 initialize-this-wall] ]

    ask patches with [ (pxcor > syringe-left-x and pxcor < syringe-right-x) and (pycor = 0 ) ]
      [ sprout-plungers 1 [ set color sky ] ]
...
to setup-axes
  ...
  create-turtles 1 [
    ...
    hatch-signs 1 [
      set size .1
      setxy xcor + 11 ycor - 2
      set label-color black
      set label "temp (> initial)"
      set axis-components (turtle-set axis-components self)
    ]
    ;bottom y axis label
    hatch-signs 1 [
      set size .1
      setxy xcor + 11 min-pycor + 2
      set label-color black
      set label "temp (< initial)"
      set axis-components (turtle-set axis-components self)
    ]
  ...
  create-turtles 1 [
    ...
    set color black
    set size 2
    setxy box-max-pxcor + margin patch-y-for 5
    set axis-components (turtle-set axis-components self)
    hatch-signs 1 [
      let v (word "+" precision logical-y-for ycor 1 " deg")
      set size .1
      setxy xcor + 6 ycor + .5
      set label-color black
      set label v;"+10u"
...
to-report patch-y-for [ logical-y ]
  report (logical-y / max-temp-delta) * max-pycor
end

to-report logical-y-for [ patch-y ]
  report ( patch-y / max-pycor ) * max-temp-delta
end

to setup-graph-run
  ifelse (particles-on-1 = 0) [
    user-message (word "<html><body><b>No Particles on Region 1 </b> </html> <html> To run the experiment, "
      "you must define Region 1 and ensure that there are particles in that Region")
    set interrupt? false
  ][
    set initial-temperature mean [energy] of particles-on region1

    set interrupt? false
    set mouse-interaction "big eraser"

    ask forces [ die ]

    ask cursors [ set xcor box-max-pxcor + margin ]
    set time-c box-max-pxcor + margin
    set current-temp initial-temperature

    set run? true
    set ready-to-run? true
  ]
end
...
to play-graph
  every .2 [
    let keep-going? true
    ifelse ( time-c < max-pxcor - margin - .5 and run? = true) [
      ask cursors [ set xcor time-c ]

      ; save current velocity
      let old-temp current-temp
      set current-temp item (round time-c) vals + initial-temperature
      ; only change temperature if there is a change (otherwise let it go as is till next tick)
      if old-temp != current-temp [
        set-ave-temp current-temp
      ]
      set time-c time-c + .1
      display
    ][
...
to do-experiment-end
  set-ave-temp initial-temperature
  set current-temp initial-temperature
  ask cursors [ set xcor box-max-pxcor + margin ]
  set ready-to-run? false
  set p-force-plot-start -1
  set plunger-plot-start -1
  display
end

to apply-temperature-change [ center deltat ]
  ask particles-on region1 [
   set energy energy + deltat
   set speed speed-from-energy
  ]
end

to set-ave-temp [atemp]
  ask particles-on region1 [
   set energy random-normal atemp 1
   set speed speed-from-energy
  ]
end
...
to prepare-for-run
  ...
  set interrupt? true
  set mouse-interaction "big eraser"
  set override-plots? true
  set-current-plot "Plunger Level"
  set-plot-x-range round (ticks + 1) round (ticks + length vals)
  set-current-plot "Forces on Syringe Plunger"
  set-plot-x-range round (ticks + 1) round (ticks + length vals)
  set-current-plot "Forces on Syringe Plunger"
  let miny (min vals) * 10 * tick-delta
  let maxy (max vals) * 10 * tick-delta
  set miny min (list -2 miny)
  set maxy max (list 2 maxy )
  set-plot-y-range round miny round maxy
  setup-graph-run
  set p-force-plot-start ticks
  set plunger-plot-start ticks
end

to-report show-initial-temp
  ifelse ready-to-run? [
    report (word precision initial-temperature 2 " deg")
  ][
    report "Experiment not yet set up"
  ]
end
...
mouse-interaction
mouse-interaction
"none - let particles move" "draw basic wall" "draw red removable wall" "draw blue removable wall" "big eraser" "slow down particles" "speed up particles" "paint particles purple" "paint particles green" "paint particles orange" "add green particles" "add purple particles" "add orange particles" "choose heat/cool region"
0
...
633
Region Volume
(word \" \" region-one-volume \" units^3\")
17
1
13
...
965
533
Identify Region to be Heated/Cooled
if any? region1 [\nask region1 [ set pcolor brown - 3 ]\ndisplay\nwait 1\nask region1 [ set pcolor black ]\n]
NIL
1
T
...
678
Region Density
(word precision (particles-on-1 / region-one-volume) 3  \" p/u^3\")
17
1
13
...
PENS
"plunger-level" 1.0 0 -14454117 true "" "if any? plungers [plotxy ticks  precision [ycor] of one-of plungers 2]"
...
623
Run Experiment
ifelse (ready-to-run? = true)\n[ \n set mouse-interaction \"none - let particles move\"\n go\n play-graph \n]\n[ \n  setup-graph-run \n  set p-force-plot-start ticks\n  set plunger-plot-start ticks\n]
T
1
T
OBSERVER
...
588
Ave Temp in Region
(word precision (mean [ energy ] of particles-on region1) 2 \" degrees\")
17
1
13
...
588
Ave Part Speed
(word precision (mean [ speed ] of particles-on region1) 2 \" u\")
17
1
13
...
1270
648
Switch to halt an ongoing experiment
11
15.0
1
...
RESTART PLOTTING - cleans and restarts the plots.
...
"choose heat/cool region" - lets you click an area above or below the plunger to be heated and/or cooled through the temp-time graph drawing.

### Running an experiment using force-time graph
...
DRAW TEMP VS TIME GRAPH - click this button to start drawing on the right sight of the view. You can to click any gray point to draw the graph. You need to click this button again once you are done drawing.
RESET AXES - Cleans the previous temp-time drawing.
IDENTIFY REGION TO BE HEATED/COOLED - flashes the region that is going to be heated and/or cooled based on the temp - time graph drawing. if you do not see any region flashing, you need to active GO/STOP/ADD ELEMENTS button, choose the "choose heat/cool region" option from the MOUSE-INTERACTION chooser, and click to an area either above or below the plunger.
PREPARE FOR EXPERIMENTAL RUN - cleans the plots and prepares the model for running the experiment based on the force-time drawing.
RUN EXPERIMENT - runs the experiment.
...
* Novak, M., Brady, C., Holbert, N., Soylu, F. and Wilensky, U. (2010).  NetLogo PNoM 5 Virtual Syringe Temperature Graph model.  http://ccl.northwestern.edu/netlogo/models/PNoM5VirtualSyringeTemperatureGraph.  Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
