...
to mouse-action
  ...
  ifelse mouse-down? [
    ask patches with [pxcor = snap-xcor and pycor = snap-ycor] [
      ...
      if mouse-interaction = "add/move (nearby particle) sensor 1"
       or mouse-interaction = "add/move (net particle) sensor 2"
       or mouse-interaction = "add/move (temperature) sensor 3"
       or mouse-interaction = "add/move (average particle) speed sensor 4" [
     ...
     ;;;; painting (recoloring) of particles by mouse / cursor actions ;;;;
      ...
      if mouse-interaction = "speed up particles"
       or mouse-interaction = "slow down particles" [
        ask eraser-window-particles [
          let max-speed 20
...
to behavior-space-setup
  ...
  set min-particle-energy 0
  set max-particle-energy 10000
  set sensor-blue-reading 0
  set sensor-red-reading 0
  set sensor-blue-total []
  set sensor-red-total []
  create-sensors 1 [set number 1 set xcor -9 set ycor 9 set size 2 set color blue]
  create-sensors 1 [set number 1 set xcor 9 set ycor -9 set size 2 set color red]
  make-box
  let wall-length 9
  let wall-height 9
  ask patches with [(pxcor > min-pxcor and pxcor < min-pxcor + wall-length and pycor = max-pycor - wall-height - 1)
   or (pxcor = min-pxcor + wall-length and pycor > max-pycor - wall-height and pycor < max-pycor)
   or (pxcor < max-pxcor and pxcor > max-pxcor - wall-length and pycor = min-pycor + wall-height + 1)
   or (pxcor = max-pxcor - wall-length and pycor > min-pycor and pycor < min-pycor + wall-length)] [
    sprout-walls 1 [
     set color gray
     initialize-this-wall
    ]
  ...
  create-particles 30 [
    ...
    set size particle-size
    set energy initial-gas-temperature
    set color-type magenta
    set color color-type
    set mass 10
    hatch-arrowheads 1 [set hidden? true create-link-from myself [tie]]
    set speed speed-from-energy
    set last-collision nobody
    setxy (min-pxcor + wall-length / 2) (max-pycor - wall-height / 2)
  ]
  create-particles 500 [
    ...
    set size particle-size
    set energy initial-gas-temperature
    set color-type orange
    set color color-type
    set mass 10
    hatch-arrowheads 1 [set hidden? true create-link-from myself [tie]]
    set speed speed-from-energy
    set last-collision nobody
    setxy (max-pxcor - wall-length / 2) (min-pycor + wall-height / 2)
  ]
  set init-avg-speed avg-speed
  set init-avg-energy avg-energy
  update-variables
...
mouse-interaction
mouse-interaction
"none - let particles move" "draw basic wall" "draw red removable wall" "draw blue removable wall" "big eraser" "slow down particles" "speed up particles" "paint particles purple" "paint particles green" "paint particles orange" "add green particles" "add purple particles" "add orange particles" "add/move (nearby particle) sensor 1" "add/move (net particle) sensor 2" "add/move (temperature) sensor 3" "add/move (average particle) speed sensor 4"
0
...
"draw red removable wall" - adds a red wall under the mouse cursor which can be alternatively removed and replaced using the REMOVE/REPLACE RED WALL button.
"draw blue removable wall" - adds a green wall under the mouse cursor which can be. alternatively removed and replaced using the REMOVE/REPLACE BLUE WALL button.
...
* Novak, M., Brady, C., Holbert, N., Soylu, F. and Wilensky, U. (2018).  NetLogo PNoM 1 Diffusion Sandbox model.  http://ccl.northwestern.edu/netlogo/models/PNoM1DiffusionSandbox.  Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
...
<!-- 2018 PNoM Cite: Novak, M., Brady, C., Holbert, N., Soylu, F. -->
@#$#@#$#@
default
true
0
...
<experiments>
  <experiment name="sample-experiment" repetitions="1" runMetricsEveryStep="true">
    <setup>behavior-space-setup</setup>
    <go>behavior-space-go</go>
    <timeLimit steps="10000"/>
    <metric>sensor-red-reading</metric>
    <metric>sensor-blue-reading</metric>
    <metric>mean sensor-red-total</metric>
    <metric>mean sensor-blue-total</metric>
    <metric>[reading] of sensors</metric>
    <enumeratedValueSet variable="initial-#-particles">
      <value value="200"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="show-wall-hits?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="pause-particles?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="magenta-sensor-function">
      <value value="&quot;speed&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="mouse-interaction">
      <value value="&quot;add/move (temperature) sensor 3&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-gas-temperature">
      <value value="250"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="visualize-particle-speed">
      <value value="&quot;arrows&quot;"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
