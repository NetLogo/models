globals [
  ...
  plunger-down-force
  plunger-down-forces
  plunger-up-force-color
  plunger-down-force-color
    ...
    [
      ...
      if mouse-down? [
        ask patches with [pxcor = snap-xcor and pycor = snap-ycor] [
          ...
          ;      if mouse-interaction = "add upward force" [
          ;        if any? plungers with [ ycor > snap-ycor and xcor = snap-xcor ]
          ;        [
          ;          let n user-one-of "How Many Newtons of Force?" [ 1 2 3 4 5 6 7 8 9 ]
          ;          sprout-forces 1 [
          ;            set xcor orig-xcor
          ;            set my-plunger one-of plungers with [ ycor > snap-ycor and xcor = snap-xcor ]
          ;            set ycor [ycor] of my-plunger - .5
          ;            set shape "force"
          ;            set heading 0
          ;            set my-f n
          ;            set color plunger-up-force-color
          ;            set size (n + 2)
          ;          ]
          ;        ]
          ;      ]
          ;
          ;
          ;      if mouse-interaction = "add downward force" [
          ;        if any? plungers with [ ycor < snap-ycor and xcor = snap-xcor ]
          ;        [
          ;          let n user-one-of "How Many Newtons of Force?" [ 1 2 3 4 5 6 7 8 9 ]
          ;          sprout-forces 1 [
          ...
          ;            set heading 180
          ;            set my-f -1 * n
          ;            set color plunger-down-force-color
          ;            set size n + 1
          ;          ]
          ;        ]
          ;      ]
...
to setup-syringe
  ...
  [
    ...
    ;  ask patches with [ (pxcor = syringe-left-x + 1 or pxcor = syringe-right-x - 1) and (pycor = syringe-bottom-y or pycor = syringe-top-y) ]
    ;  [ sprout 1 [ set breed walls set color gray - 2 initialize-this-wall] ]
    ...
    set syringe-border patches with [ (pxcor = syringe-left-x or pxcor = syringe-right-x or pxcor = syringe-left-x + 1 or pxcor = syringe-right-x - 1)
      and ((pycor >= syringe-bottom-y and pycor <= syringe-top-y) or (pycor = syringe-bottom-y - 1 or pycor = syringe-top-y + 1)) ]
...
to draw-graph
  ...
  [
    ...
    [
      ;show (word "item: " (floor ((px - (box-max-pxcor + margin))  )))
...
to paint-patches [ px py ]
  ask patches with [ pxcor = px ] ;[ (pxcor - px) <= 2 and (pxcor - px) >= 0 ]
  [
...
to-report eval-function [ xval ]
  let index  round (xval ) ;*  ( (length vals - 1) / (2 * part-radius)) )
...
to setup-graph-run
  ...
  ;set plot-moving-average? true
  ask forces [ die ]
  create-forces 1 [
    set xcor (syringe-left-x + syringe-right-x ) / 2
    set my-plunger one-of plungers with [ xcor = [xcor] of myself]
    set ycor [ycor] of my-plunger - .5
    set shape "force"
    set heading 0
    set my-f 0
  ...
  ask cursors
  [ set xcor box-max-pxcor + margin ]
  set time-c box-max-pxcor + margin
  set current-force 0

  update-arrows-for-force current-force
  ...
  every .2
    [
      ...
      [
        ...
        let old-force current-force
        set current-force item (round time-c) vals
        ;only tell the force-arrow(s) to change if desired velocity has changed.
        if (old-force != current-force) [ update-arrows-for-force current-force ]

        set time-c time-c + .1 ;+ tick-delta
                             ;show (word "time : " time-c " value " current-force)
        display
      ]
      [
        set current-force 0
...
to update-arrows-for-force [forcenow ]
  ask forces
  [
    set my-f forcenow * 10 * tick-delta  ;;5 is scaling of the graph area. tick delta is b/c this is applied over time.
    set size abs (forcenow) + 2
    set label ""; (precision forcenow 2)
...
to prepare-for-run
  ...
  set-plot-x-range round (ticks + 1) round (ticks + length vals )
  set-current-plot "Forces on Syringe Plunger"
  let min-y  (min  vals) * 10 * tick-delta
  let max-y  (max  vals) * 10 * tick-delta
  set min-y min (list -2 min-y )
  set max-y max (list 2 max-y )
...
PENS
"plunger-level" 1.0 0 -14454117 true "" "plotxy ticks  precision [ycor] of one-of plungers 2"
...
470
Particles Upwrd
precision mean plunger-up-force 2
17
1
11
...
CLEAR EXTERNAL FORCES - removes the effect of external forces drawn through the force - time graph.
...
MOUSE-INTERACTION - sets the type of interaction the user can do with the mouse in the WORLD. Possible settings include:
...
"slow down particles" - reduce the current speed of the particles by 10%.
"speed up particles" - increase the current speed of the particles by 10%.
...
DRAW FORCE VS TIME GRAPH - click this button to start drawing on the right side of the view. You can click on any gray point to draw the graph. You need to click this button again once you are done drawing.
RESET AXES - cleans the previous force-time drawing.
...
* Novak, M., Brady, C., Holbert, N., Soylu, F. and Wilensky, U. (2010).  NetLogo PNoM 4 Virtual Syringe Force Graph model.  http://ccl.northwestern.edu/netlogo/models/PNoM4VirtualSyringeForceGraph.  Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
