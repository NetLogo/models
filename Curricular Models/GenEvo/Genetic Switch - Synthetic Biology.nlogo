...
7. *ONPG molecules* – These are grey pentagons in the model. [ONPG]( https://en.wikipedia.org/wiki/Ortho-Nitrophenyl-β-galactoside) is a chemical that mimics lactose. It is normally colorless. ONPG is hydrolyzed by LacZ enzyme to produce yellow color which is used to check for enzyme activity.  Typically, [IPTG] ( https://en.wikipedia.org/wiki/Isopropyl_β-D-1-thiogalactopyranoside), another chemical that mimics lactose, is used with ONPG. For simplicity, we have not incorporated IPTG in this model. In this model, ONPG molecules bind to LacI repressor proteins that changes the shape of LacIs preventing them binding to the operator region of DNA.
