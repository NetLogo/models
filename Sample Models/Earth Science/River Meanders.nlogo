...
The model makes use of `dx` and `dy` to help replicate vector addition while still using turtles' own "heading" property, allowing for modification of a turtle's motion with both NetLogo heading-related commands as well as with vector addition.
