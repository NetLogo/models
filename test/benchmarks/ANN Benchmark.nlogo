...
to setup [ layer-counts ]
  ...
  set layers []
  let num-layers length layer-counts
  let max-layer-size max layer-counts
...
to-report apply-bools [ input ]
  let res apply-reals map [ [?1] -> ifelse-value ?1 [1][0] ] input
  report map [ [?1] -> ?1 > 0.5 ] res
end
